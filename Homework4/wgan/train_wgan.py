...
def interpolation(g_outputs, r_imgs, label_tens):
    ...
    alpha = torch.rand(g_outputs.size(0), 1, 1, 1)
...
def findGradPenalty(interpolates, d_outputs2):
    ...
    gradients = torch.autograd.grad(
        outputs = d_outputs2,
        inputs = interpolates, 
        grad_outputs = torch.ones(d_outputs2.size())
    )[0]
    ...
    for epoch in range(5):
        ...
        for real_batch, batch_labels in trainloader:
            ...
            if cnt % 20 != 0:
                ...
                interpolate_tens, red_label_tens = interpolation(g_outputs, real_batch, label_tens)
